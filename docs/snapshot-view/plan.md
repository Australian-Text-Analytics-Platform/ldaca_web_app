...
- **Eligibility surfaces as grey-out + `<DisabledReasonTooltip>`** on the Save button — same UX pattern as the Run-button disabled tooltips elsewhere in the analytic panels. The host feature computes the reason synchronously (largest block's row count, missing task, unmaterialised result, etc.) and threads it through the shared `<AnalysisFeatureHeader>`. Users see the explanation on hover *before* opening the Save dialog.
- **Materialisation is a hard prerequisite, not a soft warning.** A snapshot view is read-only — users can't re-run with a new search term, can't Process All from a snapshot, can't change tokeniser settings. Given that, the rich paginated-mode response (which carries source-document content needed only to re-extract context) is dead weight in the bundle. The Save button stays greyed-out until every selected source node has been materialised, with the disable tooltip pointing the user at Process All. Materialise compacts the result to the flat-row shape the viewer renders; the bundle becomes ~4× smaller and the load-side renderer handles exactly one row shape.
- **Re-aggregation in snapshot mode is limited to client-side helpers**. Concordance dispersion re-binning works because the bundle ships the 100-bucket histogram and the frontend re-aggregates client-side. Topic-modelling "target topic size" re-aggregation needs the embeddings + cluster assignments — those live server-side and aren't shipped, so snapshot view is fixed to the captured aggregation. Same conceptual line as re-running a concordance search with a new term: re-aggregation = changing analysis parameters = backend territory = not in snapshot scope.
...
**Materialisation is a hard prerequisite for capture.** The bundle ships the flat per-occurrence row shape produced by Process All; the unmaterialised paginated shape (per-document rows with embedded hits + full document content) is ~4× larger and contains data the read-only snapshot viewer can't act on. The Save button stays greyed-out with a `<DisabledReasonTooltip>` ("Click Process All to materialise…") until every selected node reports `materialized === true`. Capture-side defensive check in `useConcordanceSnapshotCapture` re-asserts the same predicate after the result fetch.
...
| Result rows | `POST /concordance/tasks/{id}/result` with `page_size: "all"` | JSON in v1 — flat materialised shape per node (matched_text, left/right context, doc id, start/end idx). Trim to `fullResult.data` only; the response wrapper (analysis_params, preferences, etc.) is backend metadata the viewer doesn't render. Parquet bytes are a Phase-2 size optimisation. |
| Dispersion bins per node | `/concordance/tasks/{id}/bins`, fetched in parallel with the result via `Promise.all` | Backend always returns 100 buckets; the viewer re-aggregates client-side onto `DISPERSION_DISPLAY_BIN_COUNTS = [4, 5, 10, 20, 25, 50, 100]` via `buildDispersionBinsFromBinned()`. Re-binning therefore works end-to-end with no backend access at view time. Store as `Record<nodeId, ConcordanceDispersionBinsResponse>` under `tables/dispersion-bins.json`; preserve unknown fields so future bins enhancements load cleanly. Failures to fetch one node's bins log a console warning but don't sink the capture — chart degrades to "no data" for that node. |
