...
export type { UserPreferencesUpdate } from '@/api/generated/types.gen';

export type ResolvedQuotationPreferences = Omit<GeneratedQuotationPreferences, 'engine' | 'last_remote_url'> & {
  engine: QuotationEngineConfig;
  last_remote_url: string;
};

export type ResolvedUserPreferences = Omit<
...
const normalizePreferences = (data: GeneratedUserPreferences): ResolvedUserPreferences => ({
...
export const preferencesApi = {
  get: async (headers?: Record<string, string>): Promise<ResolvedUserPreferences> => {
  ...
  update: async (body: GeneratedUserPreferencesUpdate, headers?: Record<string, string>): Promise<ResolvedUserPreferences> => {
