...
export interface UseQuotationEngineSettingsResult {
  ...
  resolvedEnginePayload: QuotationEngineConfig;
  engineReady: boolean;
  setTaskEngineConfig: (config: QuotationEngineConfig) => void;
  updateRemoteEngineId: (engineId: string) => void;
  hydrateEngineConfig: (config: QuotationEngineConfig) => void;
 ...
 * with UI memory (last remote engine id), validation feedback, request normalization,
 ...
 * together, normalize saved or user-entered ids, expose request-ready engine
...
export function useQuotationEngineSettings(): UseQuotationEngineSettingsResult {
  ...
  const hydrateEngineConfig = (config: QuotationEngineConfig) => {
    if (config.type === 'remote') {
  ...
  const resolvedEnginePayload: QuotationEngineConfig =
    engineConfig.type === 'remote'
      ? { type: 'remote', engine_id: engineConfig.engine_id.trim() }
      : { type: 'local' };

  const engineReady =
    resolvedEnginePayload.type === 'local' || resolvedEnginePayload.engine_id.length > 0;
  ...
  const buildEngineRequest = (): QuotationEngineConfig | null => {
    if (resolvedEnginePayload.type === 'local') {
      if (engineError !== null) setEngineError(null);
      return { type: 'local' };
    }

    if (!resolvedEnginePayload.engine_id.length) {
      setEngineError('Provide a remote quotation engine id.');
      return null;
    }

    const normalizedEngineId = resolvedEnginePayload.engine_id;
    if (engineConfig.type === 'remote' && engineConfig.engine_id !== normalizedEngineId) {
