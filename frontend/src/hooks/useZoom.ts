...
export const useZoom = ({ keyboardShortcuts = false, initial = 1 }: UseZoomOptions = {}) => {
  ...
  const zoomIn = useCallback(() => setZoom((z) => clamp(parseFloat((z + DOC_ZOOM_STEP).toFixed(2)))), [clamp]);
  const zoomOut = useCallback(() => setZoom((z) => clamp(parseFloat((z - DOC_ZOOM_STEP).toFixed(2)))), [clamp]);
  ...
  useEffect(() => {
    ...
    const onKeyDown = (e: KeyboardEvent) => {
      ...
      if (e.key === '+' || e.key === '=') { e.preventDefault(); setZoom((z) => clamp(parseFloat((z + DOC_ZOOM_STEP).toFixed(2)))); }
      else if (e.key === '-' || e.key === '_') { e.preventDefault(); setZoom((z) => clamp(parseFloat((z - DOC_ZOOM_STEP).toFixed(2)))); }
