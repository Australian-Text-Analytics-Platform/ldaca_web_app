...
export function SettingsDialog({ open, onOpenChange }: SettingsDialogProps) {
  ...
  const handleSaveTokenizer = () => {
    ...
    toast.success(nextModel.trim() ? 'Default tokenizer model saved' : 'Default tokenizer model cleared');
  };
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="flex h-[80dvh] w-[80vw] max-w-none flex-col gap-0 overflow-hidden p-0">
        <DialogHeader className="shrink-0 border-b border-border/60 px-6 py-4">
          <div className="flex items-start justify-between gap-4 pr-8">
            <div className="space-y-1">
              ...
              <DialogDescription>Manage saved preferences and browser-side options.</DialogDescription>
            </div>
        ...
        <Tabs defaultValue="general" orientation="vertical" className="flex min-h-0 flex-1 flex-row gap-0 overflow-hidden">
          ...
          <div className="min-h-0 min-w-0 flex-1 overflow-y-auto p-6">
            <TabsContent value="general" className="mt-0 space-y-5">
              ...
              <section className="space-y-3 border-t border-border/60 pt-4">
                ...
                <div className="flex gap-2">
                  ...
                  />
                  <Button type="button" onClick={handleSaveTokenizer}>Save</Button>
                </div>
              </section>
            </TabsContent>
              ...
              <section className="space-y-3">
                ...
                <div className="grid gap-2">
                  ...
                  <div className="flex gap-2">
                    ...
                    />
                    <Button type="button" onClick={handleSaveToken}>Save</Button>
                    <Button
                      type="button"
                      variant="outline"
                      onClick={() => {
                    ...
                    {favoriteWorkspaces.map((workspaceId) => (
                      ...
                      >
                        <div className="min-w-0">
                          <p className="truncate text-sm font-medium">{workspaceLabel(workspaceId)}</p>
            ...
            <TabsContent value="views" className="mt-0 space-y-3">
              <div>
                ...
                <p className="text-sm text-muted-foreground">Data Loader stays visible so workspaces remain reachable.</p>
              </div>
              <div className="grid gap-2 sm:grid-cols-2">
                {ALL_VIEWS.map((view) => {
                  const checked = visibleViews.includes(view);
                  const disabled = view === 'data-loader' || (checked && visibleViews.length === 1);
                  return (
                    <Label
                      key={view}
              ...
              <section className="space-y-3">
                ...
                >
                  <Checkbox
                    ...
                    onCheckedChange={(checked) => { setHintsEnabled(checked === true); }}
                  />
                  <span>Show contextual hints</span>
                </Label>
                <div className="flex flex-wrap gap-2 text-sm">
                  <Badge variant="outline">{dismissedHints.length} dismissed</Badge>
                  <Badge variant="outline">{sessionDismissedHints.size} dismissed this session</Badge>
                </div>
